    ...
    return (
        false ?
            <div className="mb35 px-3">
                <div className="row position-relative mx-auto p-3" style={{
                    width: "344px",
                    height: "150px",
                    boxShadow: "0px 0px 3px #cdd6e0, 0px 2px 8px #cdd6e0",
                    borderRadius: "8px"
                }}>
                    ...
                    <div className="col-8 px-0">
                        <div className="d-flex align-items-center">
                            <div>
                                <Icon
                                    className="mr-3 fs20 align-middle text-primary-blue material-icons-outlined">account_tree</Icon>
                            </div>
                            <div>
                                <NavLink title={title} className="h-4 text-d-none" exact to={navLinkTo}
                            ...
                            {attributes &&
                                Object.entries(attributes).slice(1).map(key => key[0] !== "node_status" &&
                                    <div className="col-md-6 px-0" key={key}>
                                        <span className="fs12 bold500 text-muted"
                                              title={key[0]}>{key[0].substr(0, 14)}</span>
                    ...
                    <div className="col-4 text-center pt-3 pr-0">
                        <h2 className="mb-2"
                            title={attributes && Object.values(attributes)[0]}>{attributes ? (Object.values(attributes)[0]).substr(0, 3) : "-"}</h2>
                        <span className="fs12 bold500 text-muted"
                              title={attributes && Object.keys(attributes)[0]}>{attributes ? (Object.keys(attributes)[0]).substr(0, 13) : "No Metric"}</span><br/>
                        <span className="fs12 bold500 text-muted py-1 px-2 rounded"
                              style={{background: '#F2F5F7'}}>{nodeCount} Nodes</span>
                    </div>

                    <div className="position-absolute h-100 node-status" style={{
                        width: "8px",
                        left: 0,
                        top: 0,
                        background: getNodeStatus(nodeStatus),
