...
import {Button as MuiButton, FormControl, Icon, MenuItem, Select as MuiSelect} from '@material-ui/core';
import {MDBCard, MDBIcon, MDBModal, MDBModalBody, MDBModalFooter,} from "mdbreact";
...
import {
    ...
    getDeepDiveAnalysis,
    updateDeepDiveAnalysis
} from "../../../services/deepDiveAnalysisService";
...
const DeepDiveAnalysis = (props) => {
    ...
    const getNodes = () => {
        kpiService.getTreeNodes(treeId).then(({data}) => {
            if (data.response[0].raw_tree !== '') {
                let nodesList = JSON.parse(data.response[0].raw_tree).map(raw => {
                    return {value: raw.nodeId, label: raw.displayName}
                });
                setNodes(nodesList || []);
            } else {
                setNodes([]);
    ...
    const getCohorts = () => {
        kpiService.getAllCohorts().then(({data}) => {
            if (data.response[0].raw_tree !== '') {
                let cohorts = data.response.map(raw => {
                    return {value: raw.cohort_id, label: raw.cohort_name}
                });
                setCohorts(cohorts || []);
            } else {
                setCohorts([]);
                                            ...
                                            {analysisList.map((v, i) => (
                                                <tr key={i}>
                                                    ...
                                                    <td className="pl15">
                                                        ...
                                                        <i className="fa fa-edit cpointer text-primary mr5"
                                                           onClick={() => openForm(v, v.uuid)}></i>
                                                        <i className="fa fa-times-circle cpointer"
                                                           onClick={() => deleteRecord(v.uuid)}
                                                           style={{color: "#ab4952"}}></i>
                                                    </span>
                                                    </td>
                                                    ...
                                                    <td>
                                                        <div className="d-flex">
                                                            <div className="my-auto">
                                                                <a href="https://idsp.incedolabs.com:9998/hub"
                                                                   target="blank">
                                                            ...
                                                            <div className="my-auto pl15">
                                                                <a href="https://idsp.incedolabs.com:8093/r/25"
                                                                   target="blank">
                                                                    {/* <MDBIcon icon="external-link-square-alt cpointer fs22" /> */}
                                                    ...
                                                    <td className="pr40 pl30">
                                                        <div className="d-flex">
                                                            <div className="my-auto flex-grow-1">
                                                                <Popover
                                                                    content={<EditAnaysisUrlForm data={v} ind={i}
                                                                                                 handleVisibleAnaysisUrlForm={handleVisibleAnaysisUrlForm}
                                                                                                 updateAnaysisUrl={updateAnaysisUrl}/>}
                                                                ...
                                                                >
                                                                    <div className="cpointer pt5 pb5"
                                                                         style={{borderBottom: "1px solid #ccc"}}>{v.analysisUrl || "Enter Analysis URL here..."}</div>
                                                                </Popover>
                                                            </div>
                                                    ...
                                                    <td className="nowrap fs13">
                                                        {v.status !== 'Draft' ? (
                                                            <span
                                                                style={{color: "green"}}>{/* <i className="fa fa-check-circle"></i>  */}PUBLISHED</span>
                                                        ) : (
                                                            <span>DRAFT</span>
                                                        )}
                                                    </td>
                                                    ...
                                                    <td className="pr15">
                                                        {v.status !== 'Draft' ? (
                                                            <MuiButton variant="outlined" className="capitalized"
                                                                       style={{
                                                                ...
                                                                //onClick={() => addAnalysisTab(v)}
                                                                       onClick={() => {
                                                                           setSelectedUri(v.analysisUrl);
                                                                           setListView(false);
                                                                       }}
                                                            >View Analysis</MuiButton>
                                                        ) : (
                                                            <MuiButton variant="outlined" className="capitalized"
                                                                       onClick={() => publish(v)} style={{
                                    ...
                                    : (isLoading) ?
                                        ...
                                        <div className="w-100 row align-items-center justify-content-center my-5">
                                            <div className="col-md-4">
                                                <img className="w300" src={`${CONTEXT}/empty-state.svg`}
                                                     alt="Empty State"/>
                                            </div>
                                            <div className="col-md-4 text-center">
                                                <h4>No Deep-dive Analysis!</h4>
                                                <p className="fs16">There are no Deep-dive analyses to show yet. A user
                                                    can create Dashboards/visualizations on third-party tools like
                                                    Tableau, Superset, etc. and show them here via an i-frame.</p>
                                            </div>
                                        </div>
                                }
                            </div>
            ...
            > */}
                    ...
                    <MDBModal isOpen={showModal} className="cascading-modal px-5 my-5" size1="fluid" size="lg">
                        <div className="py-3 px-4 d-flex align-items-center justify-content-between"
                            ...
                            <h2 className="m-0 font-weight-normal" style={{
                                color: '#1E4564',
                                fontSize: "1.5em"
                            }}>{('mode' in frmData && frmData.mode === 'POST') ? 'Create' : 'Update'} Deep Dive
                                Analysis</h2>
                            <button className="border-0 bg-white" onClick={handleCancel}><MDBIcon icon="times"/>
                            </button>
                        </div>
                            ...
                            <form onSubmit={e => {
                                ...
                                <div className="row mingap bold400">
                                    <div className="col-md-12 form-group">
                                        ...
                                        <Input value={frmData.analysisName || ''}
                                               onChange={e => handleChange(e.target.value, 'analysisName')}
                                               placeholder="Define name here"/>
                                    </div>
                                    <div className="col-md-12 form-group">
                                        <label className="req bold600">Analysis Background</label>
                                        <Input.TextArea rows="4" value={frmData.analysisDescription || ''}
                                                        onChange={e => handleChange(e.target.value, 'analysisDescription')}
                                                        placeholder="Write description of your analysis here"/>
                                    </div>
                                                ...
                                                {['bar_chart', 'stacked_bar_chart', 'pie_chart', 'show_chart', 'data_usage', 'waterfall_chart', 'area_chart']
                                                    .map((type) => (
                                                        <MenuItem key={type} value={type}
                                                                  title={type}><Icon>{type}</Icon></MenuItem>
                                                    ))}
                                            </MuiSelect>
                                        </FormControl>
                                    </div>
                        ...
                        <MDBModalFooter className="d-flex">
                            <MuiButton variant="outlined" onClick={handleCancel}
                                       style={{fontSize: "0.7rem", height: "38px"}}>Cancel</MuiButton>
                            <MuiButton variant="contained" onClick={handleOk} style={{
                                backgroundColor: "#4285f4",
                                fontSize: "0.7rem",
                                color: "#fff",
                                height: "38px"
                            }}>{('mode' in frmData && frmData.mode === 'POST') ? 'Create' : 'Update'}</MuiButton>
                        </MDBModalFooter>
                    </MDBModal>
                    {/* </Modal> */}
                </div>
                :
                <ViewAnalysisPane analysisList={analysisList} setListView={setListView} selectedUri={selectedUri}
            ...
            <div className="d-flex">
                ...
                <div className="my-auto ml-auto pl5 pr5">
                    <Button type="primary" onClick={() => {
                        props.updateAnaysisUrl(analysis_url, props.ind)
                    }}>
        ...
        <div className="bold400">
            {
                ...
                <div className="d-flex justify-content-between mb15">
                    <MuiButton variant="outlined" className="capitalized"
                               onClick={() => setListView(true)}>{`< Go back to list view`}</MuiButton>
                </div>
            }
                    ...
                    analysisList
                        ...
                        .map((v, i) => (
                            <li key={i} className="list-inline-item w-100 mr-4 cursor-pointer align-top"
                                style={(uri === v.analysisUrl) ? {
                                    ...cardStyle,
                                    borderBottom: "3px solid #4184f2"
                                } : cardStyle}
                                onClick={() => setUri(v.analysisUrl)}
                            >
                                <h6 className="font-weight-bold text-capitalize"
                                    style={{padding: "10px 10px 0"}}>{v.analysisName}</h6>
                                <div className="d-flex justify-content-center align-items-center"
                                     style={{background: "#e0e0e0", height: "160px"}}>
            ...
            <div className="position-relative">
                <div className="bg-light position-absolute w-100 text-right" style={{height: "4rem", top: 0}}>
                    <button className="border-0 bg-transparent p-4" onClick={() => setIsOpen(true)}>
                        <Icon>zoom_out_map</Icon></button>
                </div>
            ...
            <MDBModal isOpen={isOpen} className="cascading-modal px-5 my-5" size="fluid">
                <div className="py-3 px-4 d-flex align-items-center justify-content-between"
                    ...
                    <button className="border-0 bg-white" onClick={() => setIsOpen(false)}><MDBIcon icon="times"/>
                    </button>
                </div>
                <MDBModalBody className="py-4">
                    <iframe src={uri} className="w-100 border-0" style={{height: "700px"}}></iframe>
