...
import { Modal, Popover, DatePicker, Table } from "antd";
import moment from 'moment';
import uuid from 'react-uuid';
import ReactEcharts from 'echarts-for-react';
import { CONTEXT } from "../../../../config";
import { fetchExperiment, createExperiment, updateExperiment, deleteExperiment } from "../../../../services/I2AService";
                        ...
                        <Card className="d-flex border mx-auto action-card">
                            ...
                            <div className="d-flex justify-content-between align-items-top action-card-header">
                                ...
                                <div className="rounded-circle bg-primary-blue40 text-center" style={{ width: "24px", height: "24px" }}>
                                    <span className="small align-bottom text-uppercase text-white" title={props.actionCreatedBy.split(".").join(" ")}>{props.actionCreatedBy.split(".").map((n) => n[0]).join("")}</span>
                                </div>
                            </div>
                            <div>
                                <p className="font-weight-normal pr-3">{props.expDesc}</p>
                                    <h6 className="text-nowrap text-muted font-weight-normal fs12 m-0">
                                        ...
                                        <strong className="text-primary-blue pl-5">{formData.expStatus !== `DELETED` && formData.expStatus}</strong>
                                    </h6>
                            </div>
                            </div>
                        </Card>
                    ...
                    <div className="container rounded">
                        <div className="row">
                            {formData.expStatus !== 'IN-PROGRESS' && formData.expStatus !== 'CONCLUDED'
                                ...
                                : <VariantResults variants={formData.variants} expCohortPopulation={formData.expCohortPopulation} expStartDate={formData.expStartDate} expEndDate={formData.expEndDate} expStatus={formData.expStatus} />
                            }
    ...
    return <>
        <div className="col-md-12 row justify-content-between align-items-center border-bottom mx-0 mb-4">
            ...
            <div className="col-md-5 text-center">
                <h6 className="fs20 mb-1"><Icon className="align-middle mr-3" title="List">flag</Icon>{`Desired ${best['variantMetricName']} = ${best['variantDesiredGoalValue']} ${best['variantMetricUnit']}`}</h6>
                            ...
                            <div>
                                <div className="action-card-header">
                                    ...
                                    <FormControl className="w-100 my-3">
                                        <InputLabel id="demo-simple-select-label" className="label-mui">Experiment Name</InputLabel>
                                        <Select
                                    ...
                                    <Button variant="contained" className="bg-primary-blue text-white" onClick={() => setIsRecommend(false)}>Done</Button>
                                </div>
                            </div>
                        </Card>
                    }
        ...
        </div>
        {
            ...
            props.variants.map((item, i) => {
                return <div key={i} className="col-md-6 mb-4">
                    <Card className={`d-flex mx-auto mw-100 action-card ${item.variantType === `Control` ? "expanded" : "border"}`} style={{ minHeight: '145px' }}>
                        <div className="w-100">
...
const tableHeader = (variant) => {
    return [
            ...
            },
            {
                ...
                align: `center`,
                render: (_, record) => {
                    return <span className={`text-${record.status === `Most Significant` ? 'success' : 'danger'}`}>{record.status}</span>
                },
            }
