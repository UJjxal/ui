            ...
            <MDBTableBody>
                {data?.map((v, i) => (
                    <tr className="table-text-vmid" key={i}>
                        {typeof v.title !== "undefined" && <td className="pl20"><MDBIcon icon="circle"
                                                                                         className={`fa-2x ${v.status === 'Up' ? 'text-success' : 'text-danger'}`}/>
                        </td>}
                        ...
                        <td className={typeof v.title === "undefined" ? "pl20" : ""}>
                            ...
                            <MDBIcon icon={`caret-${v.status === 'Up' ? 'up' : 'down'}`}
                                     className={`fa-3x ${v.status === 'Up' ? 'text-success' : 'text-danger'}`}/>
                        </td>
                            ...
                            {v.items.length > 1 ? (
                                    ...
                                    {v.items.map((item, j) => (
                                        <li key={j}>
                                            {(props.dtlPage && item.dtlLink === true) ? (
                                                <span onClick={props.dtlPage}
                                                      className="light-blue-text cpointer">{item.title}</span>
                                            ) : (
                                ...
                                <div>
                                    {(props.dtlPage && v.items[0].dtlLink === true) ? (
                                        <span onClick={props.dtlPage}
                                              className="light-blue-text cpointer">{v.items[0].title}</span>
                                    ) : (
