    ...
    return (
        <div id="metrics-parent" style={{overflowY: 'scroll', height: '30rem'}}>
                            ...
                            <>
                                <div className="d-flex justify-content-between align-items-center my-4 pr-5">
                                    ...
                                    <Button color="primary" className="outline-none"
                                            onClick={() => props.handleRemoveThresholdClick(rowIndex)}>
                                        <Icon className="align-middle"
                                              title={`Delete Exception Threshold ${rowIndex}`}>delete_forever</Icon>
                                    </Button>
                                </div>
                                    ...
                                    {props.filterList
                                        ...
                                        .map((filterRow, filterIndex) => (
                                            <FormControl variant="outlined" className="mr-2" key={filterIndex}>
                                                <InputLabel
                                                    htmlFor="outlined-age-native-simple">{filterRow.name}</InputLabel>
                                                <Select
                        ...
                        <Grid className="d-flex flex-wrap">
                            {row.threshold.map((thres, index) => (
                                <div key={index} className="d-flex align-items-center mb-4">
                                    <SvgIcon className="mt-4 mr-2"
                                             style={{color: props.colorRange.filter(col => thres.label === col.label)[0].color}}>
            ...
            {(props.colorSetup === 'manual' && props.filterValuesAvailable) && (
                <div className="mt-1">
                    <Button color="primary" className="outline-none font-weight-bold"
                            onClick={() => props.handleAddThresholdClick()}>
