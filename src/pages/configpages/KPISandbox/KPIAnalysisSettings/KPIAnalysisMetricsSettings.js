...
const KPIAnalysisMetricsSettings = (props) => {
    ...
    return <>
        <div id="metrics-parent"
             style={{overflowY: 'scroll', height: '36rem', boxShadow: 'inset 0px -11px 11px -10px #f1f0f0'}}>
            {props.metricsList.map((metric, listIdx) => {
                return (
                    <div id="metrics-section" key={listIdx}>
							...
							<span className="ml-5 font-weight-bold d-flex flex-row align-items-end">
                                ...
                                {listIdx !== 0 ? (
                                        <Icon style={{cursor: 'pointer'}}
                                              onClick={() => props.handleRemoveMetricsClick(listIdx)}>
                                            delete
                                        </Icon>
                                    )
                        ...
                        <div className="d-flex flex-row align-items-center justify-content-between"
                             style={{width: "25.5rem", marginLeft: "5.5rem"}}>
                            <Button color="primary" onClick={() => openDrawer(listIdx, 'column')}>SELECT COLUMN</Button>
                            <Button color="primary" onClick={() => openDrawer(listIdx, 'parameter')}>SELECT
                                PARAMETER</Button>
                        </div>
                    </div>
                );
