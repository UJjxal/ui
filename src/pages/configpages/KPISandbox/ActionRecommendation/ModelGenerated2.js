...
import {MDBCard, MDBCardBody, MDBIcon, MDBModal, MDBModalBody, MDBModalFooter,} from "mdbreact";
    ...
    return (
        <div>
                        ...
                        <TabPane tab="Products" key="1">
                            <div className="row pl5 pr5">
                                {actions.map((actionName, i) => (
                                    <div key={i} className="col-md-3 cpointer pt10 pb10"
                                         onClick={() => handleActionClick(actionName)}>
                                        <MDBCard className={activeAction === actionName ? "pnc-active-card" : ""}
                                                 style={{height: '7rem', borderRadius: '1%'}}>
                                            <MDBCardBody
                ...
                <div>
                    {recommendations.map((v, i) => (
                        <div key={i} className="mb15">
                            <MDBCard>
                                <MDBCardBody>
                                    <div className="text-primary bold600 fs16 cursor-pointer"
                                         onClick={() => openRecDetail(v, i + 1)}>{v.title}</div>
            ...
            <MDBModal isOpen={showDtlModal} className="cascading-modal px-5 my-5" size="fluid" centered>
                <div className="py-3 px-4 d-flex align-items-center justify-content-between position-relative"
                     style={{borderBottom: "1px solid #dee2e6"}}>
                    <h2 className="m-0 font-weight-normal"
                        style={{color: '#1E4564', fontSize: "1.5em"}}>{recDtl.title}</h2>
                    <button className="border-0 bg-white" onClick={() => setShowDtlModal(false)}><MDBIcon icon="times"/>
                    </button>
                    ...
                    <div className="position-absolute" style={{zIndex: 100000, width: '100%', top: '-60px', left: 0}}>
                        <div className="d-flex justify-content-between container-fluid p-0">
                            <button type="button" className="btn btn-info" onClick={() => showNthRec(recDtl.n - 1)}><i
                                className="fa fa-arrow-circle-left"></i> Previous Recommendation
                            </button>
                            <button type="button" className="btn btn-info" onClick={() => showNthRec(recDtl.n + 1)}><i
                                className="fa fa-arrow-circle-right"></i> Next Recommendation
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <MDBModalBody className="py-4">
                    ...
                    <div>
                        <div className="form-group">
                            ...
                            <Input.TextArea rows="4" placeholder="Write description here" value={recDtl.dtl || ''}
                                            onChange={e => {
                                                setRecDtl({...recDtl, dtl: e.target.value})
                                            }}/>
                        </div>
                    </div>
                </MDBModalBody>
