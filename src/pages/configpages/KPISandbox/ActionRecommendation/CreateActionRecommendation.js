...
import {MDBIcon, MDBModal, MDBModalBody, MDBModalFooter, MDBTooltip} from 'mdbreact';
...
const CreateActionRecommendation = (props) => {
    ...
    const handleCheckbox = (e, menuContent, domainName, setDefault) => {
        ...
        if (!props.InitiatedFromInsightsSection) {
            ...
            const settreeList = newTreeList[0].children.map(tree => {
                return {displayName: tree.displayName, treeID: tree.treeID}
            })
        ...
        try {
            ...
            if (treeContent.data.code === 200) {
                if (treeContent.data.response[0].raw_tree !== '') {
                    let nodesList = JSON.parse(treeContent.data.response[0].raw_tree).map(raw => {
                        return {nodeId: raw.nodeId, displayName: raw.displayName}
                    });
                            ...
                            <>
                                    ...
                                    <ul className="list-inline mb-4">
                                        {
                                            KPIDomainList.map((displayName, i) =>
                                                <li key={i} className="list-inline-item">
                                                    <label
                                                        className={((domainCheckbox['name'] !== displayName) ? (`pills w-auto px-3 ${(props.InitiatedFromInsightsSection || props.selectedKPIDomain) ? 'dim1' : ''}`) : `pills w-auto px-3 bg-primary border-primary text-white`)}>
                                ...
                                <div className="form-group mt-4 pt-3">
                                    ...
                                    <div>
                                        ...
                                        {props.InitiatedFromInsightsSection ? (
                                            <div><label
                                                className="pills w-auto px-3">{selectedKpi[0].displayName}</label></div>
                                        ) : (
                                            <Autocomplete
                                ...
                                {noOfTree.current <= 1 ? (
                                    <div className="form-group mt-4 pt-3">
                                        ...
                                        {props.InitiatedFromInsightsSection ? (
                                            <div><label
                                                className="pills w-auto px-3">{selectedNode[0].displayName}</label>
                                            </div>
                                        ) : (
                                            <div>
                                                {!NodesForRecEngineTreeLoading ? <Autocomplete
                                                    ...
                                                    />
                                                    : <input disabled className="form-control w-50 h-100"
