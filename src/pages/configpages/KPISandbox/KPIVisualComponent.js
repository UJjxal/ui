...
class KPIVisualComponent extends React.Component {
    ...
    showDrawer = (node) => {
        ...
        let functionOptions = this.functionRelationOptionsNew(rndData[0]);
        this.setState({
            drawerVisible: true,
    ...
    addRelation = (nodeId, event, index, fIndex, error) => {
        ...
        event.forEach((e, index) => {
            ...
            if (newIndex >= 0) {
                ...
                exploded_event.push({
                    label: e.label.slice(newIndex + 1), id: "val-" + index, customOption: true,
    ...
    functionRelationOptionsNew = (currentRnd) => {
        ...
        if (rnds.length > 0) {
            ...
            currentRnd.dataValue.forEach(val => {
                options.push({
                    label: `this.['${val.alias ? val.alias : val.name}']`,
                    metricName: `this.['${val.name}']`
                });   // Sanjit - Show Cross Metrics
            })
            ...
            if (childIDs.length > 0) {
                ...
                childIDs.forEach(opt => {
                    ...
                    opt.dataValue.forEach(val => {
                        options.push({
                            label: `${opt.displayName}.['${val.alias ? val.alias : val.name}']`,
                            metricName: `${opt.displayName}.['${val.name}']`
                        });   // Sanjit - Show Cross Metrics
                    })
                })
                return options;
    ...
    generatePreviewTree = (rnds) => {
        ...
        const addRndChildren = (node) => {
            ...
            node.formula.forEach(f => {
                if (!f.hasOwnProperty('show')) {
                    f.show = true;
                }
            })
            node.dataValue.forEach(d => {
