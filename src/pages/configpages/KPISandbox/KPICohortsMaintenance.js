...
const KPICohortsMaintenance = (props) => {
    ...
    const columns = [
        ...
        },
        {
            ...
            key: "_description",
            render: (_, record) => {
                return <p style={{maxWidth: "230px"}}>{record.description}</p>
            }
        },
        {
            title: "Type",
    ...
    return (<>
        ...
        {isLoading ? <Loader className="mt-3 text-center"/> :
            <div className="container px-0">
                <div className="d-flex justify-content-between pb-3">
                    ...
                    <Button variant="contained" size="small" className="bg-primary-blue text-white px-3 py-1"
                ...
                {availableCohortsList.length > 0 || create ?
                    ...
                    <div className="w-100 row align-items-center justify-content-center my-5">
                        ...
                        <div className="col-md-4 text-center">
                            ...
                            <p className="fs16">There are no cohort created yet. Cohort help you track and manage
                                strategies to improve your Key Metrics. Click the New Cohort button to get started.</p>
                        </div>
                    </div>
                }
            </div>
        ...
        <Snackbar open={isAlert.status} autoHideDuration={6000}
            ...
            <Alert variant="filled" severity={isAlert?.severity ?? "success"}
                   sx={{width: '100%'}}>{isAlert.message}</Alert>
        </Snackbar>
    </>);
};
...
const InputForm = ({
    ...
    const postCohorts = () => {
        let cohorts = {
            ...formData,
            cohortType: selectedCohorts.cohortType,
            cohortFeature: JSON.stringify(selectedCohorts.cohortFeature),
            cohortId: 'cohort' + uuid().split('-').join('')
        }
    ...
    return (
        <div>
            <div className="d-flex justify-content-between border-bottom pb-3 mb-4">
                ...
                <div>
                    <IconButton className="outline-none mr-3" size="small" color="primary" aria-label="delete"
                                onClick={() => deleteCohorts([selectedItem.key])}>
                        <Icon fontSize="small">delete_outlined</Icon>
                    </IconButton>
                    <IconButton className="outline-none" size="small" color="primary" aria-label="edit"
                        ...
                        <div className="d-flex flex-column">
                            <Autocomplete
                                ...
                                renderOption={(option) => <Typography
                                    className="text-capitalize">{option.replace(/_/g, ' ')}</Typography>}
                                ...
                                onChange={(event, newValue) => setSelectedCohorts({
                                    ...selectedCohorts,
                                    cohortFeature: newValue
                                })}
                            />
                        </div>
                    }
                        ...
                        <div className="d-flex flex-column">
                            <FormControl fullWidth>
                                ...
                                <Select
                                    ...
                                    onChange={(event) => setSelectedCohorts({
                                        ...selectedCohorts,
                                        cohortFeature: [event.target.value]
                                    })}
                                    label="Features"
                                >
                                    <MenuItem value=""><em>None</em></MenuItem>
                                    {categorical && categorical.map((option, key) => (
                                        <MenuItem key={key} className="text-capitalize"
                                                  value={option}>{option.replace(/_/g, ' ')}</MenuItem>
                                    ))}
                                </Select>
                            </FormControl>
                        </div>
